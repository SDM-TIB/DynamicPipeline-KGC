...
def process_rules(file, prefix, rdf_data, predictions_folder, kg, pca_threshold):
    ...
    # Filter rules using pandas instead of pandasql
    filtered_rules = rules[(rules[pca_col] < 1) & (rules[pca_col] > pca_threshold)].copy()

    if filtered_rules.empty:
        print("No rules found meeting the PCA confidence threshold criteria.")
        return pd.DataFrame(), Graph()

    # Group by Head and count
    head_df = filtered_rules.groupby('Head').size().reset_index(name='num')
    head_df = head_df.sort_values('num', ascending=False)
    ...
    for _, val in head_df.iterrows():
        ...
        if head and isinstance(head, str):  # Check if head is valid
            ...
            # Select rules using pandas instead of pandasql
            rule_subset = filtered_rules[filtered_rules['Head'].str.contains(head, regex=False, na=False)].copy()
            rule_subset = rule_subset.sort_values(confidence_col, ascending=False)
...
def initialize(input_config):
    ...
    # Handle RDF file path
    if iteration == 1:
        # First iteration: use original path
        path = os.path.join('KG', input_data['KG'])
        rdf = os.path.join(path, input_data['rdf_file'])
    else:
        # Subsequent iterations: use the provided path directly
        rdf = input_data['rdf_file']
        path = os.path.dirname(rdf)
    ...
    print(f"Configuration loaded (Iteration {iteration}):\n"
          f"- Prefix: {prefix}\n"
          f"- Rules file: {rules}\n"
          f"- RDF file: {rdf}\n"
    ...
    logger.info(f"Configuration loaded (Iteration {iteration}):\n"
                f"- Prefix: {prefix}\n"
                f"- Rules file: {rules}\n"
                f"- RDF file: {rdf}\n"
                f"- Predictions folder: {predictions_folder}\n"
